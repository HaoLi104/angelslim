...
class Eagle3Model(nn.Module):
    ...
    def __init__(
        ...
        use_confusion_prune: bool = False,
        confusion_window: int = 16,
        confusion_threshold: float = 25.0,
    ):
        super().__init__()
        ...
        self.use_confusion_prune = use_confusion_prune
        self.confusion_tracker = (
            IntervalPerplexityTracker(confusion_window, confusion_threshold)
            if use_confusion_prune
            else None
        )
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        return cls(
            ...
            eagle_layer,
            early_stop_method,
            use_confusion_prune=use_confusion_prune,
    ...
    def eagle_generate(
        ...
        # DeepConf 思路：用轻量预测器对 draft 隐状态进行平滑，得到置信度/进度/剩余步长的运动趋势
        # early_stop_method 控制需要监控的维度，并通过这些预测值驱动后续的提前终止判定
        is_thinking = True
        ...
        for step in range(max_decode_steps):  # noqa: B007
            ...
            if self.confusion_tracker and accept_len_value > 0:
                logits_slice = logits[best_candidate, :accept_len_value]
                token_slice = candidates[best_candidate, 1 : accept_len_value + 1]
                token_slice = token_slice.to(logits_slice.device)
                token_log_probs = torch.log_softmax(logits_slice, dim=-1).gather(
                    1, token_slice.unsqueeze(-1)
                )
                token_log_probs = token_log_probs.squeeze(-1).detach().float().tolist()
                self.confusion_tracker.add(token_log_probs)
                should_stop, ppl_value = self.confusion_tracker.should_stop()
                if should_stop and is_thinking:
                    sample_token[..., -1] = self.tokenizer.stop_think_id
                    is_thinking = False
                    print(f"Perplexity Early Stop: ppl={ppl_value:.2f}")
            if is_thinking and self.tokenizer.stop_think_id in new_token_ids:
                is_thinking = False
            if is_thinking and self.early_stop_method:
                # DeepConf 判决：在 "思考" 段内，每遇到一步有效 token，就用 predictors 的平滑结果评估是否可提前结束
                can_stop = False
                ...
                if can_stop:
                    ...
                    # 一旦满足置信度/进度等阈值，DeepConf 决策即刻收束 draft 分支，并把 stop_think 标记回 target
