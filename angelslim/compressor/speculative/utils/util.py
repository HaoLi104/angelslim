...
class IntervalPerplexityTracker:
    """用于统计最近窗口的平均困惑度，并判断是否触发剪枝"""

    def __init__(self, window_size=16, threshold=25.0):
        if window_size <= 0:
            raise ValueError("window_size 必须为正数")
        self.window_size = window_size
        self.threshold = threshold
        self.log_probs = deque(maxlen=window_size)
    ...
    def add(self, log_prob_values):
        if not isinstance(log_prob_values, list):
            log_prob_values = [log_prob_values]
        self.log_probs.extend(log_prob_values)

    def should_stop(self):
        if len(self.log_probs) < self.window_size:
            return False, self.last_perplexity
        mean_log_prob = sum(self.log_probs) / len(self.log_probs)
        self.last_perplexity = math.exp(-mean_log_prob)
        return self.last_perplexity > self.threshold, self.last_perplexity
...
def update_inference_inputs(
    ...
    # drafter 会返回 early_stop_signal（由 side head 预测），这里透传给上层循环，供 DeepConf 判定是否提前退出
